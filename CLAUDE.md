...
- **세션 재판정은 "혼자서는 못 넘은 조각"끼리만 한다.** `DetectionPipeline.rescoreWithContext()`가
  같은 창의 다른 채널 조각 중 **경보·경보우려가 아닌 것**만 `[문자] …\n[통화] …` 로 이어 붙여
  문자 어댑터에 한 번 더 묻고, 결합 위험도가 80 을 넘으면서 조각 최댓값보다 30 이상 높을 때만
  `SessionEngine.fuse()`가 조각 전부를 한 세션에 넣고 ESCALATED 로 올린다(`SessionEntity.fusedRisk`).
  경보 조각을 재판정에 넣지 않는 이유 — 처음 구현은 전부 이어 붙였고, 순환 테스트에서 **정상 문자가
  앞서 온 피싱 카톡과 결합돼 그 사기 세션에 끌려 들어갔다.** 결합 점수가 높았던 것은 카톡 때문이지
  정상 문자 때문이 아니었다. 조각 각각의 `riskSignal`은 그대로다 — 세션만 다채널. 결합 텍스트 형식은 문자 어댑터 학습셋(`Voice-Detection/src/
...
**세션 재판정** — 서버가 떠 있을 때만 동작한다(키워드 대역은 결합 입력을 못 읽는다). 시뮬레이션
탭의 `택배 반송 문자` + `docs/samples/통화녹음_택배확인.txt` 조합은 조각이 0.4 / 79.6 이라
**격상되지 않는 것이 맞다** — 문자 쪽이 이미 경보우려라 재판정 대상이 아니고, 그 사건은 단독
판정과 기존 격상 로직의 몫이다. 재판정이 실제로 잡는 조합(각각 예보 이하, 이어 보면 경보)은
아직 시연 시나리오가 없다.
