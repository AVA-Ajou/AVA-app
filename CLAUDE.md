...
- **두 문턱(66 / 33)** — 위험도 하나로 자르지 않는다. 모델과 규칙 중 **몇 개가 위험하다고
  보는가**로 등급이 갈린다.

  | 위험도 | 단계 | 등급 | 화면 |
  |---|---|---|---|
  | 66 이상 | 있음 | 경보 | 빨강 `N단계 …` |
  | 66 이상 | 없음 | 경보 | 주황 `경보 · 주의` (모델만 봤다) |
  | 33~66 | 있음 | 주의보 | 주황 `주의보 · N단계 …` (규칙만 봤다) |
  | 33~66 | 없음 | — | 표시 없음 |
  | 33 미만 | — | — | 표시 없음 |

  33~66은 모델이 애매해하는 자리라 정상과 피싱이 섞인다(실측: 정상 6건 · 피싱 3건). 규칙을
  두 번째 근거로 쓰면 갈린다 — 규칙만 쓰면 정상 통화 10건 중 8건에서 신호가 켜질 만큼
  헐겁지만, **모델이 33점 이상 준 것에만 적용하니** 검증셋 정상 107건에서 주의보가 한 건도
  나오지 않았다. 66 위에서 단계를 조건으로 걸지 않는 이유는 규칙이 신호를 못 찾는 피싱이
  19%나 되기 때문이다.

  **앱 `CAUTION_THRESHOLD`(33)와 서버 `ASSESS_THRESHOLD`(33)는 같은 값이어야 한다** —
  서버가 그 아래로는 단계를 계산하지 않으므로, 앱이 더 낮으면 주의보가 영영 안 나온다.
- **진행 단계(stage)** — "지금 어디까지 왔나". **화면에 나가는 유일한 판정 표시다.**
  ...
  **신호가 하나도 안 잡히면 `null`이고 화면은 `경보 · 주의`로 낮춰 띄운다** — 모델만 위험하다고
  본 상태다. 실제로 정상 통화가 위험도 99.0을 받은 건이 여기 걸렸다. 다만 진짜 피싱의
  19%도 여기 해당하므로(검증셋 실측) **걸러내지는 않는다.**
