...
class DemoInjector(private val context: Context) {
     ...
     * 되돌렸다 — 피해자가 아직 아무 데도 신고하지 않았는데 기관이 먼저 문자를 보내는 것은
     * 앞뒤가 맞지 않고, 무엇보다 실제 수법이 그렇지 않다. 택배기사는 "확인해보세요"라고
     * 말한 뒤 **자기가 접수처를 알려준다.** 사칭은 문자의 발신자가 아니라 그 링크가 데려가는
     * 곳에서 일어난다.
     *
     * 그래서 이 시나리오는 **같은 사람이 채널을 갈아탄** 경우다. 통화로 믿음을 만들고 문자로
     * 링크를 건넨다. 조각을 따로 보면 통화는 친절한 안내고 문자는 흔한 링크 한 줄이지만,
     * 같은 시간 창에 나란히 놓으면 한 사건이 된다. 통화 쪽 전사본에서
    ...
    fun injectCardDeliveryFollowUp() {
        postDemoNotification(
            id = System.currentTimeMillis().toInt(),
            demoChannel = "SMS",
            title = "0507-1234-5678",
            text = "아까 통화드린 택배기사입니다. 말씀드린 명의도용 확인 접수처 안내드립니다. " +
                "아래 설치 링크에서 본인확인 후 접수하시면 됩니다. https://fss-report.co.kr",
        )
    }
