...
| **판정 등급** | `예보` / `주의보` / `경보우려` / `경보` (정상이면 배지 없음) |

**위험도 점수와 진행 단계는 화면에 나오지 않습니다.** 둘 다 등급을 가르는 재료로만 쓰고
DB와 로그에 남깁니다. 원본 값이 필요하면 로그에서 봅니다.
...
화면 등급은 아래 다섯입니다. 오른쪽으로 갈수록 **위험하다고 본 판정기가 늘어납니다.**
기록 탭의 `등급 설명` 버튼을 누르면 앱 안에서도 같은 안내가 펼쳐집니다.

| 정상 | 예보 | 주의보 | 경보우려 | 경보 |
|---|---|---|---|---|
| 20 미만 | 20~50, 단계 없음 | 20~50, 단계 있음 | 50~80, 단계 없음 | 80 이상 또는 50 이상+단계 |
| 표시 없음 | 노랑 | 주황 | 주황 | 빨강 |

**화면에는 등급 이름만 나갑니다.** 위험도 점수도 진행 단계도 띄우지 않습니다 — 둘 다
등급을 가르는 재료로만 쓰고 DB와 로그에 남깁니다. 단계 번호를 감추는 이유는 규칙이
"단계가 있다/없다"는 잘 가르지만 몇 단계인지까지 맞다는 보장이 없어서입니다. 틀린 번호는
"아직 2단계니까 괜찮다"는 오독을 부릅니다.

**정상을 뺀 네 등급 모두 알림이 나갑니다** — 예보도 포함입니다. 색과 이름은 판단할 근거가
얼마나 되는지를 뜻하지, 알릴지 말지를 가르지 않습니다. 예보 구간에는 정상 통화도 함께
들어오지만, 미탐은 돈이 나가고 오탐은 짜증에 그친다는 기준을 여기에도 적용했습니다.
