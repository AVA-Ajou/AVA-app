...
| `riskSignal` | `RiskSignal` | `NONE` / `FORECAST` / `CAUTION` / `HIGH_UNBACKED` / `HIGH` |
...
**정상 · 예보 · 주의보 · 경보우려 · 경보**가 `NONE` · `FORECAST` · `CAUTION` ·
`HIGH_UNBACKED` · `HIGH`에 일대일로 맞는다.

`경보`와 `경보우려`를 잠깐 같은 `HIGH` 안에서 `stage` 유무로 갈라 부르기만 했었다. 높은
위험도에서는 단계가 없어도 경보로 부르기로 하면서 그 방식이 깨졌다 — 화면이 등급 이름을
정하려면 위험도 문턱을 UI가 다시 재야 하는데, **문턱은 판정 계층 한 곳
(`BackendClassificationClient.signalOf()`)에만 있어야 한다.**

### `RiskSignal.HIGH_UNBACKED`

위험도 50~80인데 `stage`가 비어 있는 자리다. 모델만 위험하다고 봤고, 아직 단정할 만큼
높지도 않다. **알림과 세션은 `HIGH`와 완전히 같다** — 규칙이 신호를 못 찾는 피싱이 검증셋
기준 19%나 되므로 알리지 않으면 그만큼이 통째로 빠진다. 낮춘 것은 색(주황)과 이름뿐이다.

### `RiskSignal.CAUTION`

**위험도만으로는 켜지지 않는 등급이다.** 위험도가 20~50이면서 `stage`가 채워져
...
위험도가 20~50인데 `stage`가 비어 있는 자리다. 예전에는 `NONE`으로 접어 화면에서 아예
뺐는데, 여기는 **비어 있는 자리가 아니라 섞여 있는 자리다** — 실측에서 정상 6건과 피싱
3건이 함께 들어왔다. 정상으로 접으면 그 피싱 3건이 기록에서 사라져 무엇을 놓쳤는지
되짚을 수 없다.

**알림은 나간다.** `SessionEngine.ingest()`가 걸러내는 것은 `NONE` 하나뿐이라, 등급 이름이
붙은 것은 예보까지 전부 세션에 들어가고 알림을 받는다. 어느 판정기도 확신하지 않은 구간이라
같은 자리의 정상 6건도 함께 울리지만, 미탐은 돈이 나가고 오탐은 짜증에 그친다.
새 값이지만 DB 버전을 올리지 않았다 — 기존 저장 문자열(`NONE`/`CAUTION`/`HIGH`)은 그대로
읽히므로 `valueOf`가 터지지 않는다. 대신 **이 등급이 생기기 전에 분석된 행은 `NONE`으로
남아 있다** — 다시 분석해야 예보로 갱신된다.

`risk`도 `stage`도 계속 저장되지만 **화면에는 나오지 않는다.** 둘 다 `signalOf()`가 등급을
가르는 재료로만 쓰고, 사용자에게는 등급 이름만 보인다. 위험도는 값이 사실상 0 아니면 100으로
갈려 정보가 없고, 단계는 규칙이 "있다/없다"를 가를 뿐 번호까지 맞다는 보장이 없어서다.
다만 경계선 오탐(정상 통화 59.8점 같은 값)은 `risk` 컬럼으로만 보이므로 지우지 말 것.
